  ...
  Future<List<Pecera>> getAllPeceras() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<Pecera> peceras = body.map(
              (dynamic item) => Pecera.fromJson(item as Map<String, dynamic>),
        ).toList();
        return peceras;
      } else {
        print('Error en getAllPeceras: ${response.statusCode} - ${response.body}');
        throw Exception('Error al obtener las peceras del servidor.');
      }
    } catch (e) {
      print('Excepción al obtener las peceras: $e');
      throw Exception('Excepción al conectar con el servidor para obtener peceras.');
    }
  }
  ...
  Future<Pecera?> getPeceraById(int id) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('Error en getPeceraById: ${response.statusCode} - ${response.body}');
        throw Exception('Error al obtener la pecera del servidor.');
      }
    } catch (e) {
      print('Excepción al obtener la pecera con ID $id: $e');
      throw Exception('Excepción al conectar con el servidor para obtener pecera por ID.');
    }
  }
  Future<Pecera?> createPecera(Pecera pecera) async {
    try {
      ...
      if (response.statusCode == 409 || response.statusCode == 200) {
        if (responseBody.containsKey('pecera') && responseBody['pecera'] != null) {
          return Pecera.fromJson(responseBody['pecera'] as Map<String, dynamic>);
        } else if (responseBody.containsKey('status') && responseBody['status'] == 'error') {
          final String errorMessage = responseBody['message'] ?? 'Error conocido al crear la pecera.';
          print('Error de API al crear pecera: $errorMessage');
          throw Exception(errorMessage);
        } else {
          print('Respuesta inesperada del servidor (éxito pero formato desconocido): ${response.body}');
          throw Exception('Respuesta inesperada del servidor al crear la pecera.');
        }
      } else {
        final String errorMessage = responseBody['message'] ?? 'Error desconocido del servidor.';
        print('Error HTTP al crear la pecera: ${response.statusCode} - $errorMessage');
      ...
      throw Exception('Excepción al conectar con el servidor para crear pecera.');
    }
  }
  }